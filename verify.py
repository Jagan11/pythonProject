...
# directory
dir = '/mongoexport/static_files/'
dir_output = '/mongoexport/'
mongo_after_column_added = dir_output +'added_column_report.csv'

# file location from mongo raw & Mysqlraw
mysqlraw = dir +'APIs.csv'
mongoraw = sys.argv[1]
...
df0 = pd.read_csv(mongoraw, header=None)
df0.rename(columns={0: 'serviceid', 1: 'tenant', 2: 'statuscode'}, inplace=True)
df0.to_csv(mongo_after_column_added, index=False)

# read by default 1st sheet of an Excel file   # read by default 2nd sheet of an Excel file
df1 = pd.read_csv(mongo_after_column_added)
...
fmongo = dir_output + 'output_mongo' + str(now) + '.xlsx'
fmysql = dir_output + 'output_mysql' + str(now) + '.xlsx'
...
df.to_csv(dir_output + 'output_final' + str(now) + '.csv')
print(df)